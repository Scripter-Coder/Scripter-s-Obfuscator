# ScripterHub Cloudflare Worker (Stats + HIDDEN Loader Host + SERVER AUTH)
...
3. **SERVER-SIDE LICENSE AUTH (Luarmor model)** - for scripts created
   with "Require Key" ON and the Default obfuscator engine:
   - Your Users Keys sync to the worker (`sh/licenses` KV map) on every
     create/edit/ban/reset - bans and expiry changes are enforced
     **on the server**, instantly, for every existing loader.
   - At runtime the loader collects a multi-signal HWID (executor hwid,
     RbxAnalytics client id, place/job id, executor name, user id -
     hashed together so no single value can be spoofed), reads the
     license key from `getgenv().ScripterHubKey`, and calls
     `GET /sh/auth/<id>?k=KEY&h=HWID&t=<t0>`.
   - The server checks key validity, expiry, ban, and HWID lock (first
     successful auth locks the key to that hardware; sharing the key =
     `SHERR hwid`). On success it returns a **~90-second HMAC token**
     bound to that key+hwid. Only then does `/sh/k` release the split
     decryption key. No valid license = the file **cannot decrypt**.
   - **HWID reset** has a 24h server-side cooldown (`/sh/license-reset`).
   - **Kill-switch** (`/sh/killswitch`, also a button in Users Keys
     Settings): instantly fails EVERY auth - all protected loaders die
     worldwide with one click.
   - Every auth attempt (success + failure, with key/hwid/executor and
     reason) is logged to your Discord webhook.
   - Endpoint reference: `/sh/auth/:id`, `/sh/licenses` (GET/POST
     owner), `/sh/license-delete`, `/sh/license-reset`,
     `/sh/license-ban`, `/sh/killswitch`.
